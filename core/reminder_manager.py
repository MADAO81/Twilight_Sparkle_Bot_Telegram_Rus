...
class ReminderManager:
    ...
    def add_reminder(
        ...
        cursor.execute("""
            INSERT INTO reminders (user_id, chat_id, text, remind_at, is_recurring, recurring_type)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, chat_id, text, remind_at, is_recurring, recurring_type))
    ...
    def get_due_reminders(self) -> List[Dict]:
        ...
        cursor.execute("""
            SELECT id, user_id, chat_id, text, remind_at, is_recurring, recurring_type
            FROM reminders
    ...
    def mark_sent(self, reminder_id: int) -> bool:
        """Отмечает напоминание как отправленное (деактивирует)."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def reschedule_recurring(self, reminder_id: int, recurring_type: str) -> bool:
        """Переносит повторяющееся напоминание на следующий период."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if recurring_type == "daily":
            ...
            next_time = current_time + timedelta(days=30)  # упрощённо
        else:
            conn.close()
            return False
    ...
    def get_user_reminders(self, user_id: int) -> List[Dict]:
        ...
        cursor.execute("""
            SELECT id, text, remind_at, is_recurring, recurring_type
            FROM reminders
    ...
    def cancel_reminder_by_text(self, user_id: int, text_contains: str) -> bool:
